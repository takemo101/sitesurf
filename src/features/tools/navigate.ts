...
IMPORTANT: When you need to visit **multiple URLs in sequence** and extract content from each, do NOT call this tool repeatedly from the top level. Instead, drive the whole workflow from a single \`repl\` call that loops \`navigate()\` + \`browserjs()\` and saves the results with \`createOrUpdateArtifact()\`. A top-level \`navigate\` + \`read_page\` pair wastes turns and floods the AI context.
