...
export function getContextBudget(
  ...
  return {
    ...
    inputBudget,
    maxToolResultChars:
      inputBudget >= 150_000 ? 20_000 :
      inputBudget >= 80_000  ? 10_000 :
      inputBudget >= 40_000  ? 4_000  :
      inputBudget >= 20_000  ? 2_000  :
      1_000,
...
${visitedUrls.map(e =>
  `- ${e.url} (${e.title}) [${e.visitCount}x, via ${e.lastMethod ?? 'navigate'}]`
).join('\n')}
`
```
