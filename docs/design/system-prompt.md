...
**現行バージョン**: v2（モジュール型セクション構成）  
**実装**: `src/features/ai/system-prompt-v2.ts` + `src/features/ai/sections/*.ts`
...
2. Tool Philosophy        (~700 tokens)  ★ CRITICAL
3. Common Patterns        (~1,000 tokens)
4. Available Functions    (~2,400 tokens)
5. Security Boundary      (~450 tokens)  ★ セキュリティ強化
6. Completion Principle   (~100 tokens)
─────────────────────────────────────────
合計: ~4,800 tokens (-68%)
```
...
- ツール追加/変更時は必ず `src/features/ai/sections/` 内の該当セクションファイルを更新する
- ツール定義の description と system prompt の説明が矛盾しないこと
- system prompt はテスト用の fixtures にも含めて不整合を検出する
- 新しいパターンが見つかったら Section 3「Common Patterns」に追加する
