...
- Skill に `instructionsMarkdown` がある場合は short guidance (1 行サマリ)
- `activationLevel === "contextual"` のときは先頭パラグラフを最大 4 行で提示
...
| 条件                                                                 | level        | prompt 表示                                           |
| -------------------------------------------------------------------- | ------------ | ----------------------------------------------------- |
| global skill                                                         | `passive`    | `Guidance: <1 行>`                                    |
| site skill, host-only match                                          | `passive`    | `Guidance: <1 行>`                                    |
| site skill, `matchers.paths` に specific セグメントあり (path match) | `contextual` | `Guidance (contextual):` + 先頭パラグラフ (最大 4 行) |
| site skill, `matchers.paths` が catch-all (`/`, `/**` 等) のみ       | `passive`    | `Guidance: <1 行>`                                    |

既読 skill (shownSkillIds に含まれる) は token cost 安定化のため level に関わらず 1 行 `Guidance:` に揃える。
