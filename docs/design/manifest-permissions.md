...
| `alarms`        | `background/handlers/wire.ts`                                                              | wireモードのキープアライブ                                            |
...
- `extension_pages`: 拡張ページ (sidepanel.html) ではバンドル済みスクリプトのみ許可。wireモードのWebSocket接続 (`ws://localhost:*`, `ws://127.0.0.1:*`) およびHTTPS/ローカルHTTP接続を許可。
