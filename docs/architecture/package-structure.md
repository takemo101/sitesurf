...
│   │
│   ├── ai/                         # [機能] AI プロンプト管理
│   │   ├── system-prompt-v2.ts     #   システムプロンプト v2 生成
...
│   ├── DepsContext.tsx            #   (非推奨) shared/deps-context への再エクスポート
...
│   ├── index.ts                   #   Port ベースのルーター (セッションロック + パネル追跡のみ)
│   └── handlers/                  #   ハンドラ
│       ├── session-lock.ts        #   セッション排他ロック
│       ├── panel-tracker.ts       #   サイドパネルの開閉追跡
│       ├── wire.ts                #   WebSocket ブリッジ (外部 AI エージェント連携)
...
```
sidepanel/      ──→  features/*, orchestration/, adapters/*, store/*  (DI + 組み立て)
orchestration/  ──→  ports/*, features/chat (store), features/tools (定義), shared/*
...
background/     ──→  shared/port (Port 通信の型契約のみ)
全モジュール     ──→  shared/*
```
